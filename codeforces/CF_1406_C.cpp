...
//   Address :   https://codeforces.com/problemset/problem/1406/C
//   Date    :   2020/09/12
// =====================================================
...
void dfs(int u, int p, vvi &adj, int n, vi &size, vi& centroid) {
    bool is_centroid = true;
    size[u] = 1;
    for (auto &v: adj[u]) if (v != p) {
        dfs(v, u, adj, n, size, centroid);
        size[u] += size[v];
...
void solve()
{
    ...
    vi size(n+1, 0);
    vi centroid;
    dfs(1, 0, adj, n, size, centroid);
    if (centroid.size() == 1) {
        cout << centroid[0] << " " << adj[centroid[0]][0] << endl;
        cout << centroid[0] << " " << adj[centroid[0]][0] << endl;
    } else {
        for (auto &v: adj[centroid[0]]) if (v != centroid[1]) {
